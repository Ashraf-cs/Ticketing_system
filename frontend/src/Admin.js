...
function Admin(){
    ...
    useEffect(() => {
        ...
        axios.get(`http://localhost:5200/pages_content/admin/${language}`)
        .then((res) => {
        let newContent = {}
    ...
    /* Retrieving all tickets of the current month and assigning them to tickets and filteredTickets states,
    ...
    function customFilterTickets(){
        let filtered = []
        let created_at = createdAtFrom != '' ? createdAtFrom + ' 00:00:00' : '00-00-00'
        let closed_at = closedAtFrom != '' ? closedAtFrom + ' 00:00:00' : '00-00-00'
        
        if(tickets != ''){
            (tickets).forEach((ticket) => {
                if(
                    String(ticket["created_at"]) >= created_at 
                    && String(ticket["closed_at"]) >= closed_at
    ...
    if(auth == true){
        return (
            ...
            <div className="container-xxl mt-3">
                <h5 className="d-inline">{content['ticketsOf']} {date.getFullYear()}-{date.getMonth()+1}:</h5> 
                <Link to={'/tickets_list'} className="text-decoration-none mx-2" 
                    style={{color: "#0033cc", fontSize: 14+"px"}}>{content['showAll']}</Link>
                <div className="mt-4 card p-2 borer border-secondary">
                    <div className="row">
                        <div className="col">
                            <div className="card text-center fs-4 p-2 h-100">
                                <div className="card-body px-0">
                                    <p className="card-text">{content['tickets']}</p>
                    ...
                    <div className="mt-1" style={{height: 100+"%", overflow: "auto"}}>
                        ...
                        <table className="table table-hover text-center" style={{backgroundColor: "hsl(0, 0%, 97%)"}}>
                            <thead className="sticky-top" style={{backgroundColor: "hsl(0, 0%, 96%)"}}>
                                <tr>
                                    <th id="number">{content['number']}</th>
                                    <th id="createdAt">{content['createdAt']}</th>
                                    <th id="technician">{content['technician']}</th>
                                    <th id="dept">{content['dept']}</th>
                                    <th id="employee">{content['employee']}</th>
                                    <th id="phone">{content['phone']}</th>
                                    <th id="category">{content['category']}</th>
                                    <th id="description">{content['description']}</th>
                                    <th id="status">{content['status']}</th>
                                    <th id="closedAt">{content['closedAt']}</th>
                                    <th id="closedBy">{content['closedBy']}</th>
                                    <th id="comment">{content['comment']}</th>
                                    <th></th>
                                </tr>
                                {/* Filter fields  */}
                                <tr>
                                    ...
                                    <th>
                                        <input type="date" className="form-control form-control-sm" s
                                            tyle={{width: 130+"px"}} onInput={(e) => filterTickets('closed_at',e.target.value)} />
                                    </th>
                                    <th>
                                        <input type="search" className="form-control form-control-sm" 
                            ...
                            <thead className="collapse align-top" id="customFilter">
                                <tr >
                                    ...
                                    <th className="fw-normal">
                                        <input type="date" form="cFilterForm" className="form-control form-control-sm" 
                                        style={{width: 130+"px"}} onChange={(e) => setClosedAtFrom(e.target.value)} />
                                    </th>
                                    <th>
                                        <input type="search" form="cFilterForm" className="form-control form-control-sm" 
                                    ...
                                    <th>
                                        <button type="button" className="btn btn-sm btn-success" 
                                            onClick={() => customFilterTickets()} >{content['filter']}</button>
                                    </th>
                                    <th>
                                        <button type="reset" form="cFilterForm" onClick={(e) => resetFilterFields(e.target.form)} 
                                            className="btn btn-sm btn-secondary">{content['reset']}</button>
                                    </th>
                                </tr>
                            </thead>
                                        ...
                                        if(ticket.status == "open"){
                                            return(
                                                <tr key={index}>
                                                                    ...
                                                                    if(technician.username == ticket.technician){
                                                                        return(
                                                                            <option value={technician.username} 
                                                                                selected>{technician.username}</option>
                                                                        )
                                                                    }else{
                                                                        return(
                                                                            <option value={technician.username}>
                                                    ...
                                                    <td>
                                                        <div>
                                                            <button id="finish" className="btn btn-sm btn-warning" 
                                                                data-bs-toggle="modal" data-bs-target={`#finishTicket${ticket.id}`}
                                                                ...
                                                                <div className="modal-content">
                                                                    <div className="modal-header" style={{height: 45+"px"}}>
                                                                        ...
                                                                        <div className="d-flex justify-content-end">
                                                                            <button id={`ticket-close${ticket.id}`}
                                                                        ...
                                                                        <form onSubmit={(e) => e.preventDefault()}>
                                                                            <div className="row g-1">
                                                                                ...
                                                                                <div className="col">
                                                                                    ...
                                                                                    <input name="phone" value={`${ticket.phone}`} disabled
                                                                                        className="form-control form-control-sm" />
                                                                                </div>
                                                                                <div className="col-3">
                                                                                    <div className="d-flex justify-content-start">
                                                                            ...
                                                                            <div className="row">
                                                                                <div className="col">
                                                                                    ...
                                                                                    <textarea id={`comment${ticket.id}`} name="comment" 
                                                                                        rows="3" className="form-control" required
                                                                            ...
                                                                            <div className="row mt-2">
                                                                                <div className="col d-flex justify-content-end">
                                                                                    <button type="submit" style={{width: 50}}
                                                                                        className="btn btn-sm btn-warning px-1" 
